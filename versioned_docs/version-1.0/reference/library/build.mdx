...
### Step 2: Create a new Notifiles app 
Let's generate a brand new app with the CLI:

```bash
npx notifiles app new 
```
The notifiles will ask a series of questions:
...
### Add the command
```app new``` will generate a new Notifiles app project.
...
```bash
npx notifiles app new -h
```

```bash
Generate a Notifiles app 🚀
...
We can now build the command handler. Since we are going to generate a project, we'll need a good generator. We'll use the  [*chunks extension*](https://github.com/@notifiles/chunk-extension) that's made for this task.
#TODO
...
The project is available at [https://github.com/notifiles-org/generator](https://github.com/notifiles-org/generator)
